...
| `migrations/045_currency_conversion.sql` | all `tenant_%` schemas | ⏳ **pending** — adds four nullable columns to `sales`, `expenses`, and `purchases` via `run_on_all_tenant_schemas`: `original_currency text`, `original_total_amount numeric(12,2)`, `fx_rate numeric(18,8)` (CHECK `fx_rate IS NULL OR fx_rate > 0`), `fx_rate_date date`. All four null means no conversion happened (the normal case) — the existing money columns always hold base-currency amounts, so every existing aggregation/export/invoice is untouched; these four exist only to record the original figure for audit. Also mirrored into `provision_tenant_schema()` in the same commit. Fixes the confirmed live bug where 24 Swedish orders on `tenant_k2_textil`'s May 2026 Amazon report were booked as EUR instead of SEK. See `docs/superpowers/specs/2026-09-15-multicurrency-receipts-date-filters-design.md` section 1 for the full design, including the ECB rate-direction formula. Backs `src/app/dashboard/sales/`, `src/app/dashboard/expenses/`, `src/app/dashboard/purchases/`. |
...
| `migrations/047_advanced_inventory.sql` | `public` function | ✅ **applied** (2026-09-26 — defines the installer only; no tenant touched by it) — `supabase/tests/advanced_inventory.test.sql` passed on Project B (`INV_TESTS_PASSED`, 2026-09-26) — defines `public.install_advanced_inventory(schema_name)`, the idempotent installer for Business-plan batches/locations/FIFO (tables `stock_locations`, `inventory_settings`, `platform_location_defaults`, `stock_lots`, `stock_transfers`, `stock_movements`; new `purchases`/`sales` columns; triggers; RPCs `enable_advanced_inventory` (service_role), `set_default_location`, `set_opening_lot_cost`). Touches no tenant by itself. Tests: `supabase/tests/advanced_inventory.test.sql`. See `docs/superpowers/specs/2026-09-25-advanced-inventory-batches-locations-design.md`. |
| `migrations/048_advanced_inventory_apply.sql` | all `tenant_%` schemas | ⏳ **pending** — deliberately held until the PR is reviewed (existing tenants don't have the feature installed yet; new tenants get it via `005`) — runs the 047 installer on every tenant via `run_on_all_tenant_schemas`. Inert until a tenant enables it. `005`'s `provision_tenant_schema()` calls the same installer for new tenants (re-apply `005`). |
...
| `control-plane/010_tenant_shipping_labels.sql` | `control.tenants` (Project A) | ⏳ **pending** — adds `shipping_labels_enabled` boolean (default false), the per-tenant EasyPost purchasing switch. Not yet applied to the live database — apply manually via the Supabase SQL editor for Project A, or the `supabase-control` MCP tool, before Task 2's `dashboard/layout.tsx` query or Task 4 (the server-side guard) can work correctly — until then, `dashboard/layout.tsx`'s combined `select("plan, ai_enabled, shipping_labels_enabled")` will error on the missing column and silently fall back `tenantPlan`/`aiEnabled` to their defaults too, not just the new flag. See `docs/superpowers/specs/2026-09-07-shipping-label-gating-and-detail-layout-design.md`. |
