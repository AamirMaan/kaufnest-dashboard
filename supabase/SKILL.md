...
| `migrations/005_tenant_provisioning.sql` | `public` functions | ⏳ **needs re-apply** — canonical definition applied, but the `ebay_listing_drafts` table/trigger/RLS/index block added for the eBay Listing Creation feature is only in this file, not yet re-run against Project B; re-paste the full file (safe, `CREATE OR REPLACE`) so `provision_tenant_schema()` picks it up for future tenants |
...
| `migrations/021_ebay_listing_drafts.sql` | all `tenant_%` schemas | ⏳ **pending** — apply AFTER 012 (helper) is applied — creates `ebay_listing_drafts` table (draft eBay listings sourced from Inventory `product_id` OR a dropship `source_url`/`source_platform`; `status` lifecycle `draft → publishing → published/failed`) + `updated_at` trigger + `status`/`created_by` indexes + RLS (admin/super_admin only, mirrors `platform_connections`), via `run_on_all_tenant_schemas`; also baked into `provision_tenant_schema()` in `005_tenant_provisioning.sql` (table, trigger, RLS array + policy, indexes) so new tenants get it from creation. Backs the eBay Listing Creation feature (`src/app/dashboard/listings/` — Task 8's `listingsSlice.ts`, Task 12's publish route). |
