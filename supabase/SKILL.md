...
| `migrations/028_notifications.sql` | all `tenant_%` schemas | ⏳ **pending** — creates `notifications` and `notification_reads` tables (one event row per user, visibility per-reader via RLS), adds `profiles.notifications_read_through` column, with 3 indexes and grants; also baked into `provision_tenant_schema()` for future tenants. No insert/update/delete policy on `notifications` — only SECURITY DEFINER triggers write there. Backs the Notifications feature (`src/app/dashboard/notifications/`). |
