...
| `migrations/047_advanced_inventory.sql` | all `tenant_%` schemas | ⏳ **pending** — defines `public.install_advanced_inventory(schema_name)`, called per-tenant (not baked into `provision_tenant_schema()` — Business-plan opt-in feature). Creates `stock_locations`, `inventory_settings` (singleton, `id boolean = true`), `platform_location_defaults`, `stock_lots`, `stock_transfers`, `stock_movements`, adds purchases/sales columns, RLS, and a full ledger of `inv_*` SECURITY DEFINER helpers (lock/cost/shortfall/recost), purchases→lot triggers (`inv_purchase_before_write`/`inv_purchase_after_write`/`inv_purchase_before_delete`), and the one-way `enable_advanced_inventory()` RPC (EXECUTE granted to `service_role` only — called from `POST /api/inventory/enable-advanced` after plan+role checks). A closing `FOR fn IN ...` loop revokes `EXECUTE` on every `inv_%`-named function plus the three public RPCs, then re-grants each one individually. `supabase/tests/advanced_inventory.test.sql` covers it end-to-end against a throwaway `tenant_zz_invtest` schema. Not yet applied to any live database. |
